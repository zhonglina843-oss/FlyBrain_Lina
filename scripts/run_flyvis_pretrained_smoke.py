"""Run a compact FlyVis pretrained-network dynamics check."""
...
def main() -> None:
    ...
    torch.manual_seed(0)
    model_path = flyvis.results_dir / "flow/0000/000"
    network_view = flyvis.NetworkView(model_path)
    network = network_view.init_network().to(device)
    stimulus = torch.zeros(1, args.frames, 1, 721, device=device)
    stimulus[:, args.stimulus_start : args.stimulus_stop] = 1.0
    activity = network.simulate(stimulus, 1 / 50).detach()
